...
async function scenario(surface, viewport, theme = 'default') {
  ...
  await context.addInitScript(({ theme }) => {
    localStorage.setItem('sm.delivery.preferences.v2', JSON.stringify({ theme: theme === 'light' ? 'light' : 'dark', navigation: 'google', alerts: false, wake: false }));
  ...
  try {
    ...
    if (surface === 'commande') {
      ...
      await shot('compte');
      checks.push('fixture access associated', 'mission detail and Escape', 'account preferences visible; no dispatch/handoff');
...
try {
  for (const surface of ['commande', 'fidelite', 'livreur', 'restaurant', 'snackmanager']) {
    for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
      await scenario(surface, viewport);
