...
import { MODULE_ORDERING_CENTS, MODULE_ORDERING_SETUP_CENTS, PLAN_MRR_CENTS } from "@sm/contracts";
import { COMMERCE_PRICES, NEW_QUOTE_NOTE } from "./commercial-catalog";
...
describe("catalogue commercial public", () => {
  it("conserve les prix contractuels existants et compose le nouveau catalogue sans doublon", () => {
    expect(PLAN_MONTHLY_CENTS).toEqual(PLAN_MRR_CENTS);
    expect(COMMERCE_PRICES.collectMonthlyCents).toBe(MODULE_ORDERING_CENTS);
    expect(COMMERCE_PRICES.setupCents).toBe(MODULE_ORDERING_SETUP_CENTS);
  ...
  it("inclut la livraison dans Boost sans augmenter son prix", () => {
    ...
    expect(COMMERCE_TERMS.boost).toContain("Dans les nouveaux devis");
  ...
  it("affiche réellement l’inclusion Boost et conserve les limites d’exploitation", () => {
    ...
    expect(html).toContain("Dans les nouveaux devis");
    expect(html).toContain("sans supplément");
    ...
    expect(html).toContain("Sur devis, après validation de la configuration");
    expect(html).toContain(NEW_QUOTE_NOTE);
  });
});
