...
export function CustomerAccountPanel({ open, onClose, restaurantName, loyaltyHref, onDeviceOrders, account }: {
  ...
  return <Sheet open={open} onClose={onClose} title="Mon compte" navigationLocked={state.busy}
    ...
    footer={<Tap className={secondary + ' w-full'} disabled={state.busy} onClick={onClose}>
      {view ? 'Retour au restaurant' : 'Continuer en invité'}
    </Tap>}>
    <div className="space-y-5 p-4 pb-6 sm:p-5">
      {state.message && <p role="status" aria-live="polite" className="rounded-card border border-ink/10 bg-surface2 p-3 text-sm leading-6 text-ink">{state.message}</p>}
      {view ? <Profile key={`${view.profile.phoneE164}:${view.profile.phoneVerifiedAt}`} account={account} view={view} />
        ...
        </div> : <section className="rounded-panel border border-accent/20 bg-[image:var(--cf-card-gradient)] p-5">
          <span aria-hidden className="mb-3 grid size-11 place-items-center rounded-card bg-accentwash text-accentink"><Icon name="user" size={21} /></span>
          <h3 className="font-display text-xl font-extrabold tracking-tight">{needsRetry ? 'Votre compte reste à vérifier' : 'Vous pouvez commander en invité'}</h3>
          <p className="mt-3 text-sm leading-6 text-mut">{state.status === 'offline'
            ? 'Reconnectez-vous au réseau pour consulter votre profil personnel.'
            : needsRetry ? 'Nous ne pouvons pas confirmer votre session pour le moment. Votre commande reste accessible en invité.'
              : 'La création et la connexion au compte ne sont pas encore ouvertes.'}</p>
          <p className="mt-3 text-xs leading-5 text-mut">Votre carte fidélité est une carte de présentation, pas une clé de connexion à votre compte.</p>
        </section>}
      {!loading && <Tap className={secondary + ' w-full'} disabled={state.busy} onClick={() => void refresh()}>{needsRetry ? 'Réessayer' : 'Actualiser mon compte'}</Tap>}
      {(onDeviceOrders || loyaltyHref) && <nav aria-label="Vos accès au restaurant" className="space-y-2 border-t border-ink/10 pt-4">
        ...
        {loyaltyHref && (state.busy ? <span aria-disabled="true" className={secondary + ' w-full justify-start opacity-40'}><Icon name="gift" size={18} />Ma carte fidélité</span>
          : <Link href={loyaltyHref} prefetch={false} onClick={onClose} className={secondary + ' w-full justify-start'}><Icon name="gift" size={18} /><span className="flex-1">Ma carte fidélité</span><Icon name="arrow" size={15} /></Link>)}
      </nav>}
    </div>
  </Sheet>;
}
