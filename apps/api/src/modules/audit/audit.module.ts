...
export class AuditService {
  ...
  async logOnce(entry: { tenantId: string; action: 'order.refund' | 'order.refund.withdraw' | 'order.refund.allocate' | 'order.refund.allocation_withdraw' | 'order.collect' | 'order.assign' | 'order.dispatch' | 'order.handoff' | 'order.delivery_incident' | 'order.delivery_override' | 'order.delivery_proof_rotate' | Extract<TenantAuditAction, `dining.${string}` | `order.counter_refund.${string}`>; targetId: string; actor: AuditActor; meta: unknown }, operationId: string): Promise<void> {
