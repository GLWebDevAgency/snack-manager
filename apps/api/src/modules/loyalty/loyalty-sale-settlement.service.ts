...
type ObservedOrder = LoyaltyWebObservedOrder & { number: number };
const SELECT = '+loyaltyWebIntent +customerOwner +customerSaleAttribution +loyaltyMemberId +paymentFlow +counterCollection +deliveryHandoff +refundFlow';
...
export class LoyaltySaleSettlementService {
  ...
  private filter(tenantRef: string) {
    ...
    return { tenantId: new Types.ObjectId(tenantRef), channel: 'online', 'loyaltyWebIntent.version': 1 };
  }
  ...
  private query(input: HistoricalSaleSettlementInput) {
  ...
  private async project(order: ObservedOrder, actor: JwtPayload, resolutionId?: string): Promise<LoyaltySaleSettlement> {
    ...
    let input: HistoricalSaleSettlementInput;
    try { input = loyaltyWebObservation(order); }
    catch (error) {
      if (!(error instanceof LoyaltyWebObservationError)) throw error;
    ...
    const canResolve = owner(actor) && enabled() && exact && outcome.kind === 'reconciliation' && outcome.reason === 'insufficient_balance';
  ...
  async resolve(tenantRef: string, orderId: string, actor: JwtPayload, body: LoyaltySaleResolutionRequest): Promise<LoyaltySaleSettlement> {
    ...
    if (!enabled()) throw new ConflictException('Le traitement fidélité web est désactivé');
    ...
    const current = await this.historical.settleHistoricalSale(input);
    ...
    const fresh = loyaltyWebObservation(await this.order(tenantRef, orderId));
    ...
    await this.orders.updateOne({ ...this.filter(tenantRef), _id: new Types.ObjectId(orderId), 'loyaltyWebIntent.operationId': input.earnOperationId },
