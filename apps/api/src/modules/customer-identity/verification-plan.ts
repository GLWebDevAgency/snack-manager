...
export const CustomerVerificationModeSchema = z.enum(['closed_trial', 'closed_paid_pilot']);
export type CustomerVerificationMode = z.infer<typeof CustomerVerificationModeSchema>;
export type CustomerVerificationPlan = TrialSendPlan | PaidPilotSendPlan;
...
export function planCustomerPhoneVerification(input: {
  ...
  return mode.data === 'closed_trial' ? planTrialPhoneVerification(input) : planPaidPilotPhoneVerification(input);
}
