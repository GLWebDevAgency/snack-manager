...
Vérifications locales : tests contrat et schéma ; routes HTTP Nest réelles avec service simulé ; chiffrement ; vraie bibliothèque web-push avec interception HTTPS avant connexion (autorités ambiguës rejetées, chemins et paramètres opaques conservés) ; tests Mongo sur un mongod éphémère lié à 127.0.0.1 (SM_TEST_MONGOD permet de désigner le binaire, aucun MONGO_URL applicatif utilisé), notamment expiration des anciens abonnements ambigus et effacement de leurs clés ; composants React dans Chromium avec API/permission/PushManager simulés ; vrais PNG 180/192/512 et manifest/SW. Ces tests ne prouvent pas la réception sur un iPhone/Android installé, ni un envoi VAPID sur un fournisseur réel. Une recette matérielle consentie reste nécessaire sur le domaine déployé.
