...
beforeEach(async () => {
  root = await mkdtemp(path.join(os.tmpdir(), 'sm-pos-static-'));
  await writeFile(path.join(root, 'index.html'), '<main>POS</main>');
...
describe('serveur web POS', () => {
  it.each(['/%', '/%0', '/%ZZ', '/%E0%A4%A', '/%80', '/%C3%28', '/%C0%AF', '/%ED%A0%80', '/%F4%90%80%80'])(
    ...
    async (malformed) => {
      ...
      expect(await request('/')).toMatchObject({ status: 200, body: '<main>POS</main>' });
    },
  );

  it('ignore une query mal encodée et ne décode une URL valide qu’une fois', async () => {
    expect(await request('/?bad=%')).toMatchObject({ status: 200, body: '<main>POS</main>' });
  ...
  it('préserve le repli SPA, les types et les politiques de cache', async () => {
    expect(await request('/route-inconnue')).toMatchObject({ status: 200, body: '<main>POS</main>' });
