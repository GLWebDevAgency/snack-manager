...
describe('URL PostgreSQL des migrations identité client', () => {
  ...
  it('conserve le repli local pour les développeurs et tests', () => {
    expect(migrationDatabaseUrl({ NODE_ENV: 'development', DATABASE_URL: ' postgres://local ' })).toBe(
      'postgres://local',
    );
  });
