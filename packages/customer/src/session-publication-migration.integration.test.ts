...
integration('0004→0005 publication migration — limited owner, native SQL', () => {
  it('upgrades 0005 preserving legitimate legacy sessions without inventing enrollment or protection', async () => {
    ...
    try {
      ...
      expect((await fixture.admin.query('SELECT * FROM customer.session_publications')).rows).toEqual(previous);
      expect((await fixture.admin.query('SELECT 1 FROM drizzle.__drizzle_customer_migrations')).rowCount).toBe(12);
    ...
    try {
      f = await customerTestFixture(raw, { beforeUpgradeMigrations: 5, beforeUpgrade: async admin => {
        ...
        try {
          ...
          expect((await admin.query('SELECT 1 FROM drizzle.__drizzle_customer_migrations')).rowCount).toBe(12);
          expect((await admin.query(`SELECT count(*)::int AS n FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
