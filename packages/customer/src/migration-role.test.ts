...
import {
  assertCustomerMigrationRoleSafe,
  grantCustomerRuntimeRole,
  runtimeDatabaseRole,
} from './migration-role';
...
describe('rôle runtime des migrations identité client', () => {
  ...
  it('sonde le rôle puis accorde uniquement accès au schéma et CRUD', async () => {
    ...
    await grantCustomerRuntimeRole(pool, 'snackmanager_staging_app');
    ...
    expect(grants).toMatch(/GRANT USAGE ON SCHEMA customer/);
    expect(grants).toMatch(/GRANT CONNECT ON DATABASE "railway"/);
    expect(grants).toMatch(/drizzle\.__drizzle_customer_migrations/);
  ...
  it.each([
    ...
    const pool = { query } as unknown as Pick<Pool, 'query'>;
    await expect(grantCustomerRuntimeRole(pool, 'snackmanager_staging_app')).rejects.toThrow(
  ...
  it('refuse le superuser comme identité de migration avant tout DDL', async () => {
    ...
    const pool = { query } as unknown as Pick<Pool, 'query'>;
    await expect(
      assertCustomerMigrationRoleSafe(pool, 'snackmanager_staging_app'),
