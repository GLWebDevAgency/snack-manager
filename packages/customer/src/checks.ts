...
export async function settleVerification(client: PoolClient, input: Parameters<CustomerIdentityRepository['settleSend']>[0]) {
  ...
  if (row.state === 'pending') return row.verification_sid === input.verificationSid
    && row.expires_at.getTime() > await dbTime(client) ? pendingView(row) : null;
...
export async function completeVerification(client: PoolClient, input: Completion): Promise<CustomerSession | null> {
  ...
  const continuity = existing && input.existingSessionHash ? await session(client, input, input.existingSessionHash) : null;
  ...
  const created = await client.query(`WITH stamp AS MATERIALIZED (SELECT clock_timestamp() AS now)
    INSERT INTO customer.sessions(id,parent_ref,tenant_ref,account_id,session_hash,account_version,created_at,expires_at)
    SELECT $1,$2,$3,a.id,$5,a.session_version,stamp.now,LEAST($6::timestamptz,stamp.now+interval '7 days')
  ...
  [input.sessionId, input.parentRef, input.tenantRef, accountId, input.sessionHash, new Date(input.sessionExpiresAt),
    row.expires_at, existing ? input.existingSessionHash : null]);
  ...
  const result = await session(client, input, input.sessionHash);
