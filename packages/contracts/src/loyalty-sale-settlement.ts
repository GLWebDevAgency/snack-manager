...
export const LoyaltySaleSettlementSchema = LoyaltySaleAmountsSchema.extend({
...
export const LoyaltySaleSettlementQuerySchema = z.strictObject({ cursor: objectId.optional(), limit: z.coerce.number().int().min(1).max(30).default(20) });
export type LoyaltySaleSettlementQuery = z.infer<typeof LoyaltySaleSettlementQuerySchema>;

export const LoyaltySaleSettlementReadQuerySchema = z.strictObject({ resolutionId: z.uuid().optional() });
