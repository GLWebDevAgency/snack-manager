...
async function assertLoyaltyRolesSafe(
  ...
  if (
    ...
    throw new Error('Les rôles migration/runtime fidélité sont absents ou privilégiés');
  }
...
export async function assertLoyaltyMigrationRoleSafe(
  pool: Pick<Pool, 'query'>,
  role: string,
): Promise<void> {
  await assertLoyaltyRolesSafe(pool, role);
}

/** Accorde le CRUD du schéma fidélité au rôle runtime, jamais le DDL persistant. */
export async function grantLoyaltyRuntimeRole(
  pool: Pick<Pool, 'query'>,
  role: string,
): Promise<void> {
  const databaseName = await assertLoyaltyRolesSafe(pool, role);
  ...
  await pool.query(`
    ...
    GRANT USAGE ON SCHEMA loyalty TO ${quotedRole};
    GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA loyalty TO ${quotedRole};
    GRANT USAGE, SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA loyalty TO ${quotedRole};
    GRANT USAGE ON SCHEMA drizzle TO ${quotedRole};
    GRANT SELECT ON TABLE drizzle.__drizzle_loyalty_migrations TO ${quotedRole};
