...
**Deux étapes opérationnelles restent non implémentées :**

1. **Chargeur en lecture seule exhaustif** : lire Orders, toutes les admissions pertinentes et les calendriers avec projections privées explicites (`lean`, sans sérialisation publique), normaliser les identités/dates puis appeler l'analyseur. Examiner aussi les références incohérentes et états `validating/rejected`, pas seulement `committing/created`. Pagination/limite de mémoire doivent signaler un scan incomplet, jamais tronquer silencieusement. Pas de `createdAt` ou horizon public comme filtre d'occupation. La cohérence d'un scan en activité ne remplace pas la relecture sous exclusion avant bascule.
