...
Deux prochaines primitives proposées, compatibles Mongo standalone et **non implémentées** :

1. **Plan de bootstrap en lecture seule** : instant de bascule fixe, scan sans horizon maximal de toutes les Orders à créneau depuis le début du jour Paris concerné et des admissions `committing/created`, déduplication tenant/clientId/orderId. Produire grille/révision, occupation cuisine/livraison et anomalies. `ready`, `delivered`, payé ou remboursé ne libèrent pas une place : seul `cancelled` le permet. `SlotsService.countPerSlot` classe actuellement certains horaires non exacts dans le créneau précédent ; le nouveau noyau exige un slot exact. Rapporter ces écarts et les surcapacités, ne jamais les ignorer ou les arrondir silencieusement.
2. **Application reprenable du plan** : fermeture de toutes les nouvelles créations à créneau, arrêt/drainage des anciens writers, contrôle `seeding` et `bootstrapId` durables. Arbitrer les anciens `validating` par CAS, aider les `committing` insert-only sans recalculer prix/numéro/promotion, puis importer les places avec les index uniques et relecture après chaque résultat ambigu. Ne rendre journées puis tenant prêts qu'après rapprochement exhaustif. Identité discordante, snapshot/Order manquant, horaire non mappable ou surcapacité restent bloquants ; aucune suppression, annulation ou compensation promotionnelle aveugle.
...
**CI antérieure, distincte de cette passe locale :** sur `9929bcf4afb6873f50a609e926eb5b862ff779cd`, [CI 34064773193](https://github.com/GLWebDevAgency/snack-manager/actions/runs/34064773193) échoue uniquement sur le RED runtime ; le job de compilation est ignoré, pas validé par cette exécution. Le [contrôle des secrets 34064773192](https://github.com/GLWebDevAgency/snack-manager/actions/runs/34064773192) est vert. Les prochains changements doivent être vérifiés sur leur propre SHA poussé ; ce relevé ne certifie pas leur CI.
