...
Le relais Sentry applique une frontière « alertes seules » : collecte du corps HTTP désactivée, puis liste finale de champs autorisés après les processeurs SDK. Requêtes, en-têtes/cookies, contexte et breadcrumbs automatiques sont retirés ; l'URL du rapport est réduite à son chemin. Message, pile, source, version et environnement restent utiles au diagnostic. Le SDK installé a reproduit la fuite de contexte avant correction dans un transport mémoire, puis les tests ont vérifié son absence après correction dans les enveloppes complètes, sans émission fournisseur. `tracesSampleRate: 0` seul ne suffisait pas. Le DSN Sentry n'était configuré ni sur staging ni en production lors de la vérification ; aucune exposition distante par ce relais n'est attestée par cette reproduction locale.
