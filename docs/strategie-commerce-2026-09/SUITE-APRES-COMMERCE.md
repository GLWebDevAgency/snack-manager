...
**Point de reprise actuel :** L0 fusionné par [PR #120](https://github.com/GLWebDevAgency/snack-manager/pull/120), déploiement staging [33999422207](https://github.com/GLWebDevAgency/snack-manager/actions/runs/33999422207) réussi sur `e266721456afc0d188a3538c04af3e6fe1d4ac19`. La [preuve de livraison](https://github.com/GLWebDevAgency/snack-manager/pull/120#issuecomment-5555683883) distingue les tests exécutés et les limites : quatre parcours CI nécessitant des comptes ont été ignorés, contrairement à la recette Classfood faite via la session du navigateur. La production n'a pas été modifiée.

**Lot actif : C01 — reprise durable du checkout**, [PR #121](https://github.com/GLWebDevAgency/snack-manager/pull/121), branche `codex/checkout-durable-recovery`, démarrée depuis `e266721`. Critères : transaction IndexedDB avant POST ; une seule tentative par établissement/origine entre onglets ; corps métier figé ; preuve de possession aléatoire distincte du téléphone ; reprise serveur malgré pause/créneau expiré ; reçu sauvegardé avant nettoyage du panier correspondant ; suivi retrouvable panier vide. Un refus ne libère une nouvelle tentative qu'après confirmation durable du serveur ; une erreur HTTP ou un 404 n'est jamais une preuve d'absence de commande. [Protocole et limites](REPRISE-CHECKOUT-DURABLE.md). Vérifications locales : web1138, journal22, panier inter-onglets7, Mongo admission23 et paiement121 ; 9 scénarios navigateur de reprise et 5 parcours checkout existants. Revue indépendante sans blocage sur C01 ; CI et staging restent à valider, aucun nouveau GO production.
