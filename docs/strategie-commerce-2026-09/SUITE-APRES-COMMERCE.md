...
**Avancée du 7 septembre, toujours non activée :** contrôle privé et journées fermées explicites (`119c43a`), store de calendrier durable (`85bdf20`). Le plan complet est figé par CAS de révision Tenant et repris après panne, sans remplacer une journée ; l'historique non rapproché interdit son ouverture. **57 tests calendrier verts, dont49 Mongo**, plus61 tests de schéma ; passe commune grille/calendrier/noyau/C01 **201/201**, immuabilité réelle54/54. Suite API2037 verts/262 ignorés hors passes dédiées ; DB269 verts/10 ignorés hors passe Mongo dédiée. Types/lints API/DB/POS, builds API/POS et chargement du module API compilé vérifiés localement. Le RED runtime a été rejoué et reste bloquant : **2 commandes pour1 place**. Prochaine implémentation : révisions atomiques des trois writers BO, bootstrap historique puis raccordement des writers de commandes ; aucune modification de disponibilité en staging par ce lot.

La CI précédente sur `26a791e` avait échoué avant ce RED : le test POS de contrat de prix importait le serveur avant construction des types DB. `e2aa72c` ajoute la dépendance et les entrées de cache Turbo explicites. Le test conserve le vrai moteur de prix ; aucune dépendance DOM n'est ajoutée au serveur, aucun test n'est affaibli. La nouvelle CI doit être vérifiée sur le SHA poussé : les vérifications locales ne constituent pas sa preuve.
