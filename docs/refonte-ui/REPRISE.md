...
La recette automatique post-déploiement [34719365013](https://github.com/GLWebDevAgency/snack-manager/actions/runs/34719365013) a exécuté **42 tests : 37 réussis, 5 échoués, aucun ignoré**. Les quatre attentes KDS reposent sur l'ancien numéro sans dièse et l'ancien défaut/ordre des radios ; le parcours commande attend le choix d'heure avant d'avoir sélectionné le lendemain alors que la démo est fermée. Correction bornée aux cinq fichiers de recette `e2e/demo/{commande-en-ligne,cuisine,cuisine-preferences,choisir-creneau}.test.mjs` et `e2e/socle/choisir-creneau.mjs`, sans édition applicative : assertions de prix, retrait, progression, contraste et focus conservées ou renforcées. Les 42 parcours repassent sur staging ; trois contre-épreuves supplémentaires couvrent le créneau devenu chargé, complet ou désactivé. La suite finale repasse **45/45, aucun ignoré**, en 32,1 s contre staging (transport de démonstration), preuve `preuves/v2-staging-e2e-corrige-local.log`. Les six tests KDS passent aussi isolément, sans requête inattendue, ainsi que les six contre-épreuves du helper de créneau. Le parc réel n'a pas été joué par ce run ; son job est ignoré après l'échec de la démonstration.

La [PR 179](https://github.com/GLWebDevAgency/snack-manager/pull/179) est fusionnée dans `develop` en **`7e0746653b3710e98637f824e7d83e0719163527`**, le 12 septembre 2026 à 20:50:04 UTC. La tête approuvée est `a91e82429630e1789b267ac24d8bce153544026d` ; l'arbre fusionné est strictement identique (`git diff --exit-code HEAD origin/develop` réussi après fetch). [CI complète](https://github.com/GLWebDevAgency/snack-manager/actions/runs/34717459341) et [scan de secrets](https://github.com/GLWebDevAgency/snack-manager/actions/runs/34717459339) réussis sur cette tête ; toutes les étapes d'intégration configurées sont vertes, notamment Salle, paiements/remboursements, identité, commandes et livreur. Le [workflow staging](https://github.com/GLWebDevAgency/snack-manager/actions/runs/34718235062) est terminé **SUCCESS** : validations, préflight, migrations, quatre déploiements et smoke. Le smoke indépendant repasse **8/8**, avec la révision API exacte `7e0746653b3710e98637f824e7d83e0719163527`, 22 catégories / 109 produits publics, catalogue et PWA fidélité, web/POS/KDS. L’API Railway `7d887342-d1a8-4a58-b0d3-b20cce38e998` confirme à 21:09:37 UTC : « Schémas et index du service à table préparés. » Voir `preuves/v2-staging-{workflow.json,smoke.log,api-bootstrap.json}`. Les reçus GitHub sont `preuves/v2-pr179-{merge,ci,secrets}.json`.

Le premier scan de secrets a signalé 11 empreintes SHA-256 de sources (10 valeurs distinctes, toutes recalculées au commit de preuve) et deux URI loopback inventées pour vérifier le refus des identifiants avant I/O. La règle générique reçoit une liste fermée de ces seules empreintes dans les trois manifestes exacts ; les deux fixtures utilisent désormais la construction URL et seules leurs occurrences historiques précises sont documentées dans `.gitleaksignore`. Aucune règle générale ni aucun test n'est retiré. Cinq contrôles synthétiques de la portée de l'exception réussissent ; les deux suites Salle/pricing repassent sur Mongo local : **59/59**, avec typecheck API et lint ciblé réussis. Les scans locaux de l'historique et de l'arbre complet passent, puis GitHub confirme les deux scans. Les empreintes de source antérieures restent les preuves de leurs exécutions ; seuls ces deux fichiers de test diffèrent depuis, sans changement applicatif. Voir `preuves/v2-gitleaks-canaries.json`.

Worktree actif : `/Users/limameghassene/development/SnackManager-refonte-ui`. Branche du lot initial : **`refactor/ui-handoff-fidelity`** ; branche de suivi de livraison : **`fix/e2e-refonte-staging`**. La PR 178 est fusionnée dans `develop` en **`80f2eac593863dfcfc1f9e5962c09db38fe6db24`** ; les commits locaux V2 ont été rebasés dessus sans conflit. Le travail concurrent du checkout principal reste préservé. L'utilisateur a autorisé la publication, la fusion vers `develop` et le déploiement staging automatique (« ok assure toi de merge afin que je teste sur staging, fais cela proprement go »). Cette autorisation ne concerne pas `main` ni la production.
